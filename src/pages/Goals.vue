<template>
  <div class="goals">
    ...
    <form @submit.prevent="addNewGoalHandler" class="goal-form">
      ...
      <input v-model.number="goalAmount" type="number" placeholder="Сумма" required class="input-field" />
      <button type="submit" class="submit-button">Добавить Цель</button>
    </form>

    <ul>
      <li v-for="goal in goals" :key="goal.id">
        <strong>{{ goal.title }}</strong> ({{ goal.category }}): {{ goal.amount }} UA
...
export default {
  ...
  setup() {
    ...
    const categories = ref(['Сбережения', 'Путешествия', 'Образование', 'Покупки']); // пример категорий

    const addNewGoalHandler = async () => {
      const goalId = Date.now().toString(); // Создаем уникальный ID в виде строки
      await budgetStore.addGoal({
        id: goalId, // Присваиваем ID цели
    ...
    const deleteGoalHandler = async (goalId) => {
      console.log(`Удаление цели с ID: ${goalId}`); // Логируем ID для отладки
